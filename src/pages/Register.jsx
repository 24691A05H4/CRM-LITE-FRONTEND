    ...
    <div className="min-h-screen flex bg-[#1C1C24] font-sans">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 z-30 bg-[#1C1C24]">
        <div className="w-full max-w-md">
          <h2 className="text-[32px] font-semibold text-white mb-3">Create an account</h2>
          <p className="text-[#8F8F9D] text-sm mb-10">
            Already have an account?{' '}
            <Link to="/login" className="text-[#8E75FF] hover:text-[#7A61E6] underline underline-offset-4 transition-colors">
              Log in
            </Link>
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex gap-4">
              <input
                ...
                className="flex-1 w-full px-4 py-3.5 bg-[#252530] border border-[#353542] rounded-xl text-white placeholder-[#6B6B78] focus:outline-none focus:border-[#8E75FF] focus:ring-1 focus:ring-[#8E75FF] transition-all duration-200"
              />
              <input
                type="password"
                required
            ...
            <div className="flex items-center gap-3 mt-4 mb-2">
              <button 
                ...
                className={`w-5 h-5 rounded-md flex items-center justify-center transition-colors ${agreed ? 'bg-white' : 'bg-[#252530] border border-[#353542]'}`}
              >
                {agreed && <Check size={14} className="text-[#1C1C24] font-bold" />}
              </button>
              <span className="text-sm text-[#8F8F9D]">
                I agree to the <span className="text-[#8E75FF] underline cursor-pointer">Terms & Conditions</span>
              </span>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3.5 bg-[#7B5EE4] hover:bg-[#6A4DD4] disabled:opacity-70 text-white rounded-xl font-medium transition-all duration-200 flex justify-center items-center gap-2"
            >
              {isLoading ? (
                <>
          ...
          <div className="mt-8 flex items-center gap-4">
            ...
            <span className="text-[#6B6B78] text-xs font-medium uppercase tracking-wider">Or register with</span>
            <div className="flex-1 h-px bg-[#353542]"></div>
          </div>
